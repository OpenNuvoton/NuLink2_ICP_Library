 ...
 * @file     main.c
 * @version  V1.00
 * @brief    Show how to use M0A21 ICP lib to erase/program/read target flash.
 *
...
int32_t main(void)
{
    ...
    printf("+-------------------------------------+\n");
    printf("|    M0A21 ICP Library Sample Code    |\n");
    printf("+-------------------------------------+\n\n");
    ...
    /* Use Reset mode to enter ICP mode */
    bEntry = ICP_ModeEntry(ICP_RESET_PIN_RESET_MODE);

    if (!bEntry)
    {
        /* If failed, Use GPIO mode */
        ICP_ModeExit();

        bEntry = ICP_ModeEntry(ICP_RESET_PIN_GPIO_MODE);
    }
    ...
    if (bEntry)
    {
        ...
        /* Re-enter ICP mode */
        ICP_ModeEntry(ICP_RESET_PIN_RESET_MODE);
